...
type Props = { pool: PoolData; locale: string }

export function PoolCardCMS({ pool, locale }: Props) {
  const imageSrc =
    pool.image && typeof pool.image === 'object' && 'url' in pool.image
      ? (pool.image.url ?? '/static-images/event-image-1.webp')
      : '/static-images/event-image-1.webp'

  const imageAlt =
    pool.image && typeof pool.image === 'object' && 'alt' in pool.image
      ? (pool.image.alt ?? pool.cardTitle ?? 'Pool')
      : (pool.cardTitle ?? 'Pool')
    ...
    <div className="overflow-hidden rounded-[14px] border-2 border-[var(--benefits-border)] bg-white">
      <div className="grid md:grid-cols-2">
        <div className="relative min-h-[220px] overflow-hidden">
          <PoolCardImage src={imageSrc} alt={imageAlt} />
          <div
