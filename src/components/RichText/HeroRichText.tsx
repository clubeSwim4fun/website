/**
 * RichText renderer for the Hero block.
 * Extends the default converters with a custom `text` converter that reads
 * TextStateFeature colour state (stored under the "$" key in the serialized
 * node) and applies the matching inline style.
 */
...
// Colour map — must match the state keys defined in heroLexical.ts
...
const jsxConverters: JSXConvertersFunction<NodeTypes> = ({ defaultConverters }) => ({
  ...
  // Override text converter to apply TextStateFeature colour
  text: ({ node, nodesToJSX, parent }) => {
    const stateKey = (node as any)['$']?.color as string | undefined
    const style = stateKey ? COLOR_STYLES[stateKey] : undefined

    // Reproduce what the default text converter does, but with optional colour
...
export default function HeroRichText(props: Props) {
